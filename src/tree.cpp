...
void tree::pr(bool pc) 
{
  ...
  Rcpp::Rcout << pad << "(id,parent): " << id << sp << pid;
  Rcpp::Rcout << sp << "(v,c): " << v << sp << c;
  Rcpp::Rcout << sp << "theta: " << theta;
...
void tree::birthp(tree_p np,size_t v, size_t c, double thetal, double thetar, double temp_mr)
{
  tree_p l = new tree;
  l->theta=thetal;
  tree_p r = new tree;
...
void tree::deathp(tree_p nb, double theta, double& temp_mr)
{
  delete nb->l;
  delete nb->r;
  nb->l=0;
